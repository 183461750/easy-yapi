...
open class EcsbSuvApiExporter {
    ...
    abstract class ApiExporterAdapter {
        ...
        private var suvApiExporter: EcsbSuvApiExporter? = null

        fun setSuvApiExporter(suvApiExporter: EcsbSuvApiExporter) {
...
//            builder.bind(Logger::class, "delegate.logger") { it.with(ConsoleRunnerLogger::class).singleton() }
    ...
    class EcsbApiExporterAdapter : ApiExporterAdapter() {

        @Inject
        protected val ecsbSettingsHelper: EcsbSettingsHelper? = null

        override fun actionName(): String {
            return "EcsbExportAction"
        }
        ...
        override fun doExportDocs(docs: MutableList<Doc>) {
            val suvEcsbApiExporter = actionContext.init(SuvEcsbApiExporter())

            try {
                docs.forEach { suvEcsbApiExporter.exportDoc(it) }
            } catch (e: Exception) {
    ...
    companion object {
        ...
        private val EXPORTER_CHANNELS: List<*> = listOf(
            ApiExporterWrapper(EcsbApiExporterAdapter::class, "Ecsb", Request::class, MethodDoc::class),
