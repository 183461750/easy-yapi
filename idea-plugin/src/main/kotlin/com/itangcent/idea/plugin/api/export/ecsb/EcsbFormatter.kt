...
open class EcsbFormatter {
    ...
    fun methodDoc2Item(methodDoc: MethodDoc): HashMap<String, Any?> {
        ...
        if (methodDoc.ret != null) {
            ...
            if (ecsbSettingsHelper.ecsbResBodyJson5()) {
    ...
    fun request2Items(request: Request): List<HashMap<String, Any?>> {
        ...
        if (urls.single()) {
            ...
            ruleComputer.computer(EcsbClassExportRuleKeys.AFTER_FORMAT, suvRuleContext, request.resource())
...
//        item["path"] = formatPath(request.path)
        ...
        if (request.body != null) {
            ...
            if (ecsbSettingsHelper.ecsbReqBodyJson5()) {
                item["req_body_is_json_schema"] = false
    ...
    fun item2Request(item: HashMap<String, Any?>): Request {
        ...
        ActionContext.getContext()?.instance(TipsHelper::class)?.showTips(ECSB_BODY_PARSE_TIP)
...
//        item["properties"] = HashMap<String, Any?>()
    ...
    companion object {
        ...
        // TODO: support parse ecsb body
        private val ECSB_BODY_PARSE_TIP = OnlyOnceInContextTip(
            "Ecsb body not be parsed. It will be supported in the future"
        )
    }
}