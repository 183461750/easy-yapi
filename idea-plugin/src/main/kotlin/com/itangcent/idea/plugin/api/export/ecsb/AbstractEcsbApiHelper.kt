...
abstract class AbstractEcsbApiHelper : EcsbApiHelper {
    ...
    open fun getProjectWeb(module: String): String? {
        val token = ecsbSettingsHelper.getPrivateToken(module, false) ?: return null
        val projectId = getProjectIdByToken(token) ?: return null
        return "${ecsbSettingsHelper.getServer()}/project/$projectId/interface/api"
    }
    ...
    override fun getProjectIdByToken(token: String): String? {
        if (ecsbSettingsHelper.loginMode()) {
            return token
        }
    ...
    override fun getProjectInfo(token: String, projectId: String?): JsonObject? {
        ...
        var url = "${ecsbSettingsHelper.getServer(false)}$GET_PROJECT_URL?token=$token"

        var rawProjectId = projectId
        if (ecsbSettingsHelper.loginMode() && rawProjectId == null) {
    ...
    private fun Map<String, String>.getToken(): String? {
        ...
        if (module != null) {
            return ecsbSettingsHelper.getPrivateToken(module, false).also {
            ...
            synchronized(this)
            {
                if (!init) {
                    ruleComputer!!.computer(
                        EcsbClassExportRuleKeys.BEFORE_EXPORT, SuvRuleContext(),
                        null
                    )
                    init = true
                }
